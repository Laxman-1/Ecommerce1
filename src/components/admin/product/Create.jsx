...
const Create = ({ placeholder }) => {
  ...
  const [sizes, setSizes] = useState([]); // Uncommented sizes state
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch sizes (Uncommented sizes fetch)
        const sizesRes = await fetch(`${apiUrl}/sizes`, {
          headers: { Authorization: `Bearer ${adminToken()}` }
        });
  ...
  <div className='container'>
    <div className='row'>
      <div className='d-flex justify-content-between mt-5 pb-3'>
        <div className='h4 pb-0 mb-0'>Products / Create</div>
        ...
        <form onSubmit={handleSubmit(saveProduct)}>
          <div className='row'>
            <div className='col-md-12'>
              ...
              <div>
                {sizes.map((size) => (
                  <div key={`size-${size.id}`} className="form-check">
                    <input
                    ...
                    {gallery.map((image) => (
                      <div className='col-md-3 mb-3' key={image.id}>
                        <div className='card shadow position-relative'>
                          <img src={image.url} alt="Gallery" className='card-img-top' />
                          <button 
                            type='button'
